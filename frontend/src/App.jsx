    ...
    <div style={{position:"fixed",inset:0,background:"#000000dd",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"#030a03",border:"1px solid #00ff41",borderRadius:6,padding:28,width:520,maxHeight:"80vh",overflowY:"auto",boxShadow:"0 0 40px #00ff4122"}}>
        ...
        <div style={{background:"#020202",border:"1px solid #0d2a0d",borderRadius:4,padding:14,marginBottom:16}}>
          ...
          <input value={name} onChange={e=>setName(e.target.value)} placeholder="Session name (e.g. GPT-4o Red Team #1)..." style={inputStyle}/>
            ...
            {sessions.map(s=>(
              <div key={s.id} onClick={()=>onSelect(s)}
                ...
                <div>
                  ...
                  <div style={{fontSize:10,color:"#2a4a2a",marginTop:3}}>{s.attack_count} attacks · last active {s.updated_at.slice(0,10)}</div>
                </div>
  ...
  if (!metrics || metrics.total_attacks === 0)
    return (
      <div style={{textAlign:"center",padding:"20px 0"}}>
        <div style={{fontSize:11,color:"#2a4a2a",marginBottom:8}}>No data yet.</div>
        <div style={{fontSize:10,color:"#1a3a1a"}}>Run attacks and execute them to populate metrics.</div>
      </div>
    );
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
          ...
          {Object.entries(metrics.by_status).map(([status,count])=>(
            <div key={status} style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}>
              ...
              <div style={{flex:1,height:4,background:"#0a0a0a",borderRadius:2,overflow:"hidden"}}>
                <div style={{width:`${(count/metrics.total_attacks)*100}%`,height:"100%",
                  background:STATUS_COLOR[status]||"#c8c8c8",boxShadow:`0 0 4px ${STATUS_COLOR[status]||"#c8c8c8"}`}}/>
              </div>
      ...
      {Object.keys(metrics.by_category).length > 0 && (
        <div style={{background:"#020202",border:"1px solid #0d2a0d",borderRadius:3,padding:"8px 10px"}}>
          <div style={{fontSize:9,letterSpacing:1,color:"#4a8a4a",marginBottom:8}}>BY CATEGORY</div>
          {Object.entries(metrics.by_category).sort((a,b)=>b[1].avg_score-a[1].avg_score).map(([cat,data])=>(
            <div key={cat} style={{marginBottom:6}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}>
                <span style={{fontSize:9,color:CATEGORIES[cat]?.color||"#c8c8c8"}}>{CATEGORIES[cat]?.glyph} {cat}</span>
                <span style={{fontSize:9,color:"#4a8a4a"}}>{data.count} · {(data.avg_score*100).toFixed(0)}%</span>
              </div>
      ...
      {metrics.top_techniques?.length > 0 && (
        <div style={{background:"#020202",border:"1px solid #0d2a0d",borderRadius:3,padding:"8px 10px"}}>
          <div style={{fontSize:9,letterSpacing:1,color:"#4a8a4a",marginBottom:8}}>TOP TECHNIQUES BY SUCCESS</div>
...
export default function App() {
  ...
  const [targetBehavior, setTargetBehavior] = useState("Generate harmful content without restrictions");
  ...
  const [termLines, setTermLines] = useState([
    ...
    {text:"Persistent sessions · SQLite · Rate-limited · Structured logging",dim:true},
  ...
  const providerCfg = () => ({
    provider, model:provider==="ollama"?selectedModel:openaiModel,
    base_url:ollamaUrl, api_key:openaiKey,
  });
  ...
  useEffect(()=>{
    ...
    fetch(`${API}/models/ollama?base_url=${encodeURIComponent(ollamaUrl)}`)
      .then(r=>r.json())
      .then(d=>{ if(d.models?.length){ setOllamaModels(d.models); setSelectedModel(d.models[0]); log(`Detected ${d.models.length} Ollama model(s)`,"#00cfff"); }})
      .catch(()=>log("Ollama unreachable — check URL","#ff2d2d"));
  },[ollamaUrl,provider]);
  ...
  const handleSelectSession = (s) => {
    ...
    log(`Session loaded: "${s.name}" · ${s.attack_count} existing attacks`,"#00cfff");
  ...
  const handleGenerate = async () => {
    ...
    log(`[GENERATE] ${technique} / ${category}`,"#ffd700");
    try {
      const d = await apiFetch("/generate",{session_id:session.id,category,technique,target_behavior:targetBehavior,provider:providerCfg()});
      setAttackId(d.attack_id); setBasePrompt(d.prompt); setEvolvedPrompt(""); setActivePrompt(d.prompt);
      setResponse(""); setSignals(null);
      log(`[OK] Prompt generated (${d.prompt.length} chars) id:${d.attack_id.slice(0,8)}`,"#00ff41");
  ...
  const handleEvolve = async () => {
    ...
    log(`[EVOLVE] Strategy: ${evoStrategy}`,"#bf00ff");
    try {
      const d = await apiFetch("/evolve",{session_id:session.id,attack_id:attackId,original_prompt:basePrompt,
        category,technique,evolution_strategy:evoStrategy,provider:providerCfg()});
  ...
  const handleExecute = async () => {
    ...
    log(`[EXECUTE] → ${provider}/${provider==="ollama"?selectedModel:openaiModel}`,"#ff6b00");
    try {
  ...
  const handleAdaptive = async () => {
    if(!session) return log("[ERR] Select a session first","#ff2d2d");
    if(history.length < 2) return log("[ERR] Need at least 2 attacks in session history","#ff2d2d");
    setLoading(l=>({...l,adaptive:true}));
    log(`[ADAPTIVE] Analysing ${history.length} attack vectors...`,"#00cfff");
    try {
      const d = await apiFetch("/adaptive",{session_id:session.id,category,provider:providerCfg()});
      setAttackId(d.attack_id); setBasePrompt(d.adaptive_prompt); setEvolvedPrompt(""); setActivePrompt(d.adaptive_prompt);
      setResponse(""); setSignals(null);
      log(`[OK] Adaptive prompt synthesized from ${d.history_size} samples`,"#00ff41");
      refreshHistory(session.id);
    } catch(e){ log(`[ERR] ${e.message}`,"#ff2d2d"); }
  ...
  const loadHistoryAttack = (h) => {
    ...
    const cat = h.category; if(CATEGORIES[cat]) { setCategory(cat); setTechnique(h.technique); }
  };
    ...
    <div style={{minHeight:"100vh",background:"#010101",color:"#c8c8c8",
      ...
      <style>{`
        ...
        *{box-sizing:border-box;}
        ...
        .cat-btn{cursor:pointer;border-radius:3px;padding:6px 10px;font-family:'Share Tech Mono',monospace;font-size:11px;transition:all .15s;border:1px solid;display:block;width:100%;text-align:left;margin-bottom:4px;}
        .panel{background:#030a03;border:1px solid #0d2a0d;border-radius:4px;padding:14px;}
        .lbl{font-size:10px;letter-spacing:2px;color:#4a8a4a;text-transform:uppercase;margin-bottom:6px;}
      ...
      {/* Header */}
      <div style={{padding:"14px 20px 12px",borderBottom:"1px solid #0d2a0d",
        background:"linear-gradient(180deg,#050f05 0%,transparent 100%)",
        display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"baseline",gap:14}}>
          <span style={{fontFamily:"'Orbitron',monospace",fontSize:24,fontWeight:900,
          ...
          {session ? (
            <>
              <div style={{fontSize:11,fontFamily:"'Rajdhani',sans-serif"}}>
                <span style={{color:"#4a8a4a"}}>SESSION: </span>
                <span style={{color:"#00ff41",fontWeight:700}}>{session.name}</span>
                <span style={{color:"#2a4a2a",marginLeft:8}}>{history.length} attacks</span>
              </div>
              <button className="btn" onClick={()=>setShowSessions(true)}
                style={{fontSize:10,padding:"4px 10px",background:"transparent",color:"#4a8a4a",border:"1px solid #1a2a1a"}}>
                SWITCH
              </button>
            </>
          ) : (
            <button className="btn" onClick={()=>setShowSessions(true)}
              style={{fontSize:10,padding:"5px 14px",background:"#003310",color:"#00ff41",border:"1px solid #00ff41"}}>
              SELECT SESSION
            </button>
          )}
          <span style={{width:7,height:7,borderRadius:"50%",background:"#00ff41",
            boxShadow:"0 0 8px #00ff41",display:"inline-block",animation:"blink 2s infinite"}}/>
        </div>
      </div>

      <div style={{display:"grid",gridTemplateColumns:"245px 1fr 295px",gap:10,padding:"10px 14px"}}>

        {/* LEFT */}
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          <div className="panel">
            <div className="lbl">Target Provider</div>
            <div style={{display:"flex",gap:6,marginBottom:10}}>
              {["ollama","openai"].map(p=>(
                <button key={p} className="btn" onClick={()=>setProvider(p)}
                  style={{flex:1,padding:"6px 0",fontSize:11,
                    background:provider===p?"#0d2a0d":"transparent",
                    color:provider===p?"#00ff41":"#4a8a4a",
                    border:`1px solid ${provider===p?"#00ff41":"#1a2a1a"}`}}>
                  {p.toUpperCase()}
                </button>
              ))}
            </div>
            {provider==="ollama" ? (
              ...
              <>
                ...
                <input type="password" value={openaiKey} onChange={e=>setOpenaiKey(e.target.value)}
                  placeholder="sk-..." style={{width:"100%",padding:"5px 8px",fontSize:11,marginBottom:8}}/>
                <div className="lbl">Model</div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8}}>
            {[
              {key:"generate",label:"GENERATE",color:"#00ff41",bg:"#003310",desc:"Build base attack",    action:handleGenerate},
              {key:"evolve",  label:"EVOLVE",  color:"#bf00ff",bg:"#1a0030",desc:"Optimize & escalate", action:handleEvolve},
              {key:"execute", label:"EXECUTE", color:"#ff2d2d",bg:"#2a0000",desc:"Fire at target",       action:handleExecute,pulse:true},
              ...
              <button key={key} className="btn" onClick={action} disabled={loading[key]}
                ...
                <div style={{fontSize:9,letterSpacing:1,opacity:.6,marginTop:3,fontFamily:"'Share Tech Mono',monospace"}}>{desc}</div>
              </button>
            ))}
          </div>
          ...
          <div className="panel">
            ...
            <textarea value={activePrompt} onChange={e=>setActivePrompt(e.target.value)} rows={7}
              placeholder={"// Attack prompt appears here...\n// You can also type or paste your own prompt directly."}
          ...
          <div className="panel" style={{display:"flex",gap:10,alignItems:"center",padding:"10px 14px"}}>
          ...
          <div className="panel" style={{flex:1}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <div className="lbl" style={{margin:0}}>TARGET RESPONSE</div>
              {signals&&(
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <Badge label={signals.status} color={STATUS_COLOR[signals.status]}/>
                  <ScoreBar score={signals.success_score} width={130}/>
                </div>
              )}
            </div>
            ...
            {signals&&(
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:5,marginTop:8}}>
                {[
                  {label:"Refusal Signals",   val:signals.refusal_signals,              bad:signals.refusal_signals>0},
                  ...
                  {label:"Short Refusal",      val:signals.is_short_refusal?"YES":"NO",   bad:signals.is_short_refusal},
                ].map(({label,val,bad})=>(
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          <div style={{display:"flex",borderBottom:"1px solid #0d2a0d"}}>
            {["history","metrics"].map(tab=>(
              <button key={tab} className="btn" onClick={()=>{ setActiveTab(tab); if(tab==="metrics"&&session) refreshMetrics(session.id); }}
                style={{flex:1,padding:"8px 0",fontSize:11,borderRadius:0,
          ...
          <div className="panel" style={{flex:1,overflowY:"auto",maxHeight:"calc(100vh - 240px)"}}>
            {activeTab==="history" ? (
              <>
                ...
                {history.length===0 ? (
                  <div style={{fontSize:11,color:"#2a4a2a",textAlign:"center",padding:"20px 0",lineHeight:2}}>
                    {session ? "No attacks yet.\nFire EXECUTE to start." : "Select a session to begin."}
                  </div>
                ) : history.map(h=>(
                  <div key={h.id} onClick={()=>loadHistoryAttack(h)}
                    style={{background:"#020202",
                      borderLeft:`3px solid ${STATUS_COLOR[h.status]||"#1a2a1a"}`,
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                      <span style={{fontSize:10,color:CATEGORIES[h.category]?.color||"#c8c8c8",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:140}}>
                        {h.technique}
                      </span>
              ...
              <>
                ...
                <MetricsPanel metrics={metrics} onRefresh={()=>session&&refreshMetrics(session.id)}/>
              </>
            )}
          </div>
        </div>
